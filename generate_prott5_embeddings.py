...
=============================
Generates ProtT5 embeddings for both train and test protein sequences.
Outputs:
    - train_prott5_embeddings.npy
    - train_protein_ids.npy  (same IDs, saved again for convenience)
    - test_prott5_embeddings.npy
    - test_protein_ids.npy

Run on HPC with:
    srun --partition=gpus --nodelist=thor --gres=gpu:1 --time=480 \
        singularity run cafa.sif generate_prott5_embeddings.py
...
def generate_embeddings(sequences_dict, description=""):
    ...
    ProtT5 requires spaces between each amino acid character.
    """
    ...
    print(f"Generating ProtT5 embeddings for {len(protein_ids)} {description} proteins...")
...
np.save(os.path.join(OUTPUT_DIR, "train_prott5_embeddings.npy"), train_embeddings)
np.save(os.path.join(OUTPUT_DIR, "train_protein_ids.npy"), np.array(train_ids))
print(f"Saved train ProtT5 embeddings: {train_embeddings.shape}")

test_embeddings, test_ids = generate_embeddings(test_seqs, "test")
np.save(os.path.join(OUTPUT_DIR, "test_prott5_embeddings.npy"), test_embeddings)
np.save(os.path.join(OUTPUT_DIR, "test_protein_ids.npy"), np.array(test_ids))
print(f"Saved test ProtT5 embeddings: {test_embeddings.shape}")

print("\nDone! Files saved to:", OUTPUT_DIR)
print("  train_prott5_embeddings.npy")
print("  train_protein_ids.npy")
print("  test_prott5_embeddings.npy")
